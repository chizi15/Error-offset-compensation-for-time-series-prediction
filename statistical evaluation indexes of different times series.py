"""
对各层级预测值及补偿值的监控，得到多种统计指标并对预测结果和补偿结果分类。

总体说明：
一、 可分别传入不同类别如企业、门店、课、品类群、分类等在当前周的补偿值、预测值、真实值序列，以统计不同类别下的的补偿状况。
    以一段时间如一个季度为滑动窗口，滚动计算三类序列间的统计指标如RMSE、MAPE、R2等，以评估该滑动窗口内的补偿状况。
二、 根据传入的补偿值、预测值、真实值在当前周的关系，将外层条件分为三大类：1.补偿后变好，2.补偿后变差，3.补偿后无效果；
    并得到这三大类的6个总体指标。
三、 在第一大类即补偿后变好的情况中，又分为六小类，分别是：1.1 补偿发生在同侧且变好，预测值及补偿值均低于真实值；
    1.2 补偿发生在同侧且变好，预测值及补偿值均高于真实值；1.3 补偿发生在对侧且变好，预测值低于真实值，补偿值高于真实值；
    1.4 补偿发生在对侧且变好，预测值高于真实值，补偿值低于真实值；1.5 完全准确补偿，预测值低于真实值；1.6 完全准确补偿，预测值高于真实值；
    并得到这六小类共计(6+3)*2=18个总体指标。
四、 在第二大类即补偿后变差的情况中，又分为六小类，分别是：2.1 补偿发生在同侧且变差，预测值及补偿值均低于真实值；
    2.2 补偿发生在同侧且变差，预测值及补偿值均高于真实值；2.3 补偿发生在对侧且变差，预测值低于真实值，补偿值高于真实值；
    2.4 补偿发生在对侧且变差，预测值高于真实值，补偿值低于真实值；2.5 完全准确预测，补偿值低于真实值；2.6 完全准确预测，补偿值高于真实值；
    并得到这六小类共计(6+3)*2=18个总体指标。
五、 对于第一及第二大类中的每一小类，均有预测偏差量、补偿后剩余偏差量、补偿量这三类指标，每类指标又分为8个绝对量统计指标和8个相对量统计指标，
    所以每小类中共计3*2*8=48个分项指标。
六、 所以每次运行程序，共得到6+18+18+48*(6+6)=618个统计指标；而在字典result1中共会存储1+6+18+6*6+18+6*6=115个指标变量。
七、 当y、yhat、ycomp是从所有单品中取值组成的序列时，当在第一大类中出现（y-ycomp==0），不能直接区分没有补偿和完全准确补偿的情况；
    当在第二大类中出现（y-yhat==0），不能直接区分没有预测和完全准确预测的情况；
    当在第三大类中出现（y-yhat==y-ycomp==0），不能直接区分没有预测、没有补偿、完全预测、完全补偿这四种情况；
    基于上述原因，会导致错误统计某些分项的指标，并导致无法通过检查条件的情况出现；
    建议y、yhat、ycomp从所有发生补偿的单品中取值，将没有发生补偿的单品排除。
"""
...
def cei(y, yhat, ycomp):
    ...
    if len(y) == len(yhat) == len(ycomp):  # 若该条件满足，则必有：len(y_s1)==len(yhat_s1)==len(ycomp_s1), \
        ...
        if mt == k1:
            ...
            raise Exception('计算有误，变好、变差、无效果三种情况的单品数相加与发生补偿的单品总数不等。')
    else:
