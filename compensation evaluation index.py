...
k1 = 5000
y_test = pd.Series(random.choices(range(0, 100), k=k1, weights=weights))  # 生成随机序列，模拟k1个序列在某一周的真实销量

z1 = pd.Series(np.sin(random.choices(range(0, 100), k=k1)))  # 假定这k1个是都有预测销量的序列
yhat_test = y_test + z1

z2 = pd.Series(np.sin(random.choices(range(0, 100), k=k1)))  # 并且假定这k1个是都发生补偿的序列
...
def cei(y, yhat, ycomp):
    ...
    if len(y) == len(yhat) == len(ycomp):  # 若该条件满足，则必有：len(y_s1)==len(yhat_s1)==len(ycomp_s1), \
        ...
        # 因为这里只需获取三大类情况各自的序列数量m1/m2/m3，所以从y、yhat、ycomp任一序列中选取序列，得到的m1/m2/m3都相等。\
        # 下面是从y序列中选取序列，得到的yhat序列的长度m1/m2/m3.
        m1, m2, m3 = len(y_s1), len(y_s2), len(y_s3)
        mt = m1 + m2 + m3
        if mt == k1:
            x1, x2, x3 = m1 / mt * 100, m2 / mt * 100, m3 / mt * 100
        else:
            raise Exception('计算有误，变好、变化、无效果三种情况的序列数相加与发生补偿的序列总数不等。')
    else:
    ...
    if m11 + m12 + m13 + m14 + m15 + m16 == m1:
        ...
        raise Exception('计算有误，当补偿变好时，其下6种子情况的序列总数相加与补偿变好的序列总数不等。')

    # 1.1 预测补偿均低，补偿同侧变好：
    pva_s1_c1 = abs(y_s1_c1 - yhat_s1_c1)  # 预测偏差量
    pva_s1_c1_sts = pva_s1_c1.describe()
    pvr_s1_c1 = abs(pva_s1_c1[y_s1_c1 != 0] / y_s1_c1[
        y_s1_c1 != 0]) * 100  # 预测偏差量与真实销量百分比，在此时的条件下，真实值(分母)为0的序列不能参与该指标计算，\
    # 否则会使下面describe的统计指标失真。所以pvr_s1_c1计算的是真实销量不为0的那些序列。
    ...
    cvr_s1_c1 = abs(cva_s1_c1[y_s1_c1 != 0] / y_s1_c1[
        y_s1_c1 != 0]) * 100  # 补偿后剩余偏差量与真实销量百分比，在此时的条件下，真实值(分母)为0的序列不能参与该指标计算，\
    # 否则会使下面describe的统计指标失真。所以cvr_s1_c1计算的是真实销量不为0的那些序列。
    ...
    if m21 + m22 + m23 + m24 + m25 + m26 == m2:
        ...
        raise Exception('计算有误，当补偿变差时，其下6种子情况的序列总数相加与补偿变差的序列总数不等。')
...
result1 = \
    {'第一层条件，判断总体补偿情况':
        {'本次统计发生补偿的序列总数': a[0],
         '补偿变好的序列数': a[1],
         '变好序列占全部发生补偿的序列百分比': a[2],
         '补偿变差的序列数': a[3],
         '变差序列占全部发生补偿的序列百分比': a[4],
         '补偿无效果的序列数': a[5],
         '无效果序列占全部发生补偿的序列百分比': a[6]
         },
     '第一大类情况，即补偿变好时':
        {'第一大类中六个子类的总体指标':
            {'预测销量与补偿后销量均比真实销量低，补偿变好且发生在同侧，该种情况的序列数': a[7],
             '均低同侧变好占补偿变好序列数的百分比': a[8],
             '预测销量与补偿后销量均比真实销量高，补偿变好且发生在同侧，该种情况的序列数': a[9],
             '均高同侧变好占补偿变好序列数的百分比': a[10],
             '预测销量偏低，补偿后销量偏高，补偿变好且发生在对侧，该种情况的序列数': a[11],
             '预低补高对侧变好占补偿变好序列数的百分比': a[12],
             '预测销量偏高，补偿后销量偏低，补偿变好且发生在对侧，该种情况的序列数': a[13],
             '预高补低对侧变好占补偿变好序列数的百分比': a[14],
             '预测销量偏低，完全补偿，该种情况的序列数': a[15],
             '预低全补占补偿变好序列数的百分比': a[16],
             '预测销量偏高，完全补偿，该种情况的序列数': a[17],
             '预高全补占补偿变好序列数的百分比': a[18],
             '补偿变好且发生在同侧的序列数': a[19],
             '补好同侧序列数占补偿变好序列数的百分比': a[20],
             '补偿变好且发生在对侧的序列数': a[21],
             '补好对侧序列数占补偿变好序列数的百分比': a[22],
             '完全补偿的序列数': a[23],
             '完全补偿的序列数占补偿变好序列数的百分比': a[24]
             },
     ...
     '第二大类情况，即补偿变差时':
        {'第二大类中六个子类的总体指标':
            {'预测销量与补偿后销量均比真实销量低，补偿变差且发生在同侧，该种情况的序列数': a[61],
             '均低同侧变差占补偿变差序列数的百分比': a[62],
             '预测销量与补偿后销量均比真实销量高，补偿变差且发生在同侧，该种情况的序列数': a[63],
             '均高同侧变差占补偿变差序列数的百分比': a[64],
             '预测销量偏低，补偿后销量偏高，补偿变差且发生在对侧，该种情况的序列数': a[65],
             '预低补高对侧变差占补偿变差序列数的百分比': a[66],
             '预测销量偏高，补偿后销量偏低，补偿变差且发生在对侧，该种情况的序列数': a[67],
             '预高补低对侧变差占补偿变差序列数的百分比': a[68],
             '预测销量偏低，完全补偿，该种情况的序列数': a[69],
             '预低全补占补偿变差序列数的百分比': a[70],
             '预测销量偏高，完全补偿，该种情况的序列数': a[71],
             '预高全补占补偿变差序列数的百分比': a[72],
             '补偿变差且发生在同侧的序列数': a[73],
             '补好同侧序列数占补偿变差序列数的百分比': a[74],
             '补偿变差且发生在对侧的序列数': a[75],
             '补好对侧序列数占补偿变差序列数的百分比': a[76],
             '完全补偿的序列数': a[77],
             '完全补偿的序列数占补偿变差序列数的百分比': a[78]
             },
